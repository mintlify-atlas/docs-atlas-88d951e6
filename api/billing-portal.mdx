...
<CodeGroup>
```bash cURL
curl -X POST "https://api.autumn.com/customers/cus_123/billing_portal?return_url=https://example.com/account" \
...
curl -X POST "https://api.autumn.com/customers/cus_123/billing_portal" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
const response = await fetch('https://api.autumn.com/customers/cus_123/billing_portal', {
  method: 'POST',
  headers: {
...
response = requests.post(
  'https://api.autumn.com/customers/cus_123/billing_portal',
  headers={
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json'
  },
...
const getBillingPortal = async (
  ...
  const response = await fetch(
    `https://api.autumn.com/customers/${customerId}/billing_portal`,
    {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer YOUR_API_KEY',
